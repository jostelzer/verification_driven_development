...
Place a compact snapshot near the top of the report:
...
- The snapshot must agree with the manifest and the detailed sections below.
- Use the human-step badge only for whether a remaining human verification step exists, not as a generic blocker signal.

## Certificate

- Use the heading `## 🏅 Verification Certificate`.
- Keep VERIFIED certificates terse.
- Put `How YOU Can Run This` immediately below the certificate in the final chat response.
